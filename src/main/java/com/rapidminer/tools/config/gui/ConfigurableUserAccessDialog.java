...
public class ConfigurableUserAccessDialog extends ButtonDialog {
	...
	private static final Icon LEFT_ICON = SwingTools.createIcon("24/nav_left_green.png");
	private static final Icon RIGHT_ICON = SwingTools.createIcon("24/nav_right_green.png");
		...
		ProgressThread pt = new ProgressThread("load_user_groups") {
			...
			public void run() {
				...
				try {
					...
					allGroups = new ArrayList<>(ConfigurationManager.getInstance().getPermittedGroupsForConfigurable(
							configurable));
				}

				final Set<String> permittedGroups = ConfigurationManager.getInstance().getPermittedGroupsForConfigurable(
						configurable);
