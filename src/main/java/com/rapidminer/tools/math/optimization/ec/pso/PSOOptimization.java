...
public abstract class PSOOptimization implements Optimization {
	...
	public void optimize() throws OperatorException {
		...
		while (!((population.getGeneration() >= maxGen)
				|| (population.getGenerationsWithoutImprovement() >= maxWithoutImprovement) || (population
				.getBestFitnessEver() >= population.getBestPerformanceEver().getMainCriterion().getMaxFitness()))) {
			// update velocities
