...
public class ConfigurableValueCellEditor extends AbstractSuggestionBoxValueCellEditor {
	...
	public ConfigurableValueCellEditor(final ParameterTypeConfigurable type) {
		...
		if (!hasSaveDestination) {
			...
			try {
				processRepository = openProcess.getRepositoryLocation().getRepository();
		...
		configButton.setIcon(SwingTools.createIcon("16/"
				+ ConfigurationManager.getInstance().getAbstractConfigurator(typeId).getIconName()));
