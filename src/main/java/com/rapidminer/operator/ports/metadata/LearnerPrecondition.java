...
public class LearnerPrecondition extends CapabilityPrecondition {
	...
	protected Collection<QuickFix> getFixesForClassificationWhenRegressionSupported() {
		...
		QuickFix fix = new OperatorInsertionQuickFix("insert_classification_by_regression_learner", new Object[] { name,
				learner.getOperatorDescription().getName() }, 3, getInputPort()) {
	...
	protected Collection<QuickFix> getFixesForPolynomialClassificationWhenBinominalSupported() {
		...
		QuickFix fix = new OperatorInsertionQuickFix("insert_binominal_to_multiclass_learner", new Object[] { name,
				learner.getOperatorDescription().getName() }, 8, getInputPort()) {
